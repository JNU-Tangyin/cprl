...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a remote forecaster and export forecast_full.npz.")
    ...
    parser.add_argument("--cache_save_path", required=True)
        ...
        for batch_x, batch_y, batch_x_mark, batch_y_mark in loader:
            ...
            if exp.args.use_amp:
                with torch.cuda.amp.autocast():
                    res = exp.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
            else:
                res = exp.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
            ...
            if isinstance(res, (tuple, list)):
                ...
                outputs = res.get("pred", res.get("output", res.get("outputs")))
            else:
                outputs = res
            ...
            if data_set.scale and getattr(exp.args, "inverse", False):
...
def main() -> int:
    ...
    out_path = Path(args.cache_save_path)
...
def maybe_skip_existing(out_path: Path) -> bool:
    if out_path.exists():
        print(f"[Cache] skip existing: {out_path}")
        return True
    return False
