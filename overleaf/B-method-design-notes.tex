...
\paragraph{Our design.}
We do not have a quantile regression model, but we can approximate the
base quantiles \emph{empirically} from the calibration buffer.  Let
$r_i = \tilde{e}_i^{+} - \tilde{e}_i^{-} = (Y_i - \hat{Y}_i) /
\hat{\sigma}_i$ be the signed normalised residual.  We compute running
quantile estimates:
\begin{equation}\label{eq:base-quantiles}
  \hat{q}_{\mathrm{lo}} = Q_{\alpha/2}(r_{1:n}),
  \qquad
  \hat{q}_{\mathrm{hi}} = Q_{1-\alpha/2}(r_{1:n}),
\end{equation}
which serve as the ``pseudo quantile regression'' output (constant in $x$
but adaptive in time via the sliding calibration window).

The CQR nonconformity score is then:
\begin{equation}\label{eq:cqr-score}
  E_i = \max\!\bigl(\hat{q}_{\mathrm{lo}} - r_i,\;
  r_i - \hat{q}_{\mathrm{hi}}\bigr).
\end{equation}
Note that $E_i \le 0$ when $r_i \in [\hat{q}_{\mathrm{lo}},
\hat{q}_{\mathrm{hi}}]$, and $E_i > 0$ otherwise.  The single conformal
threshold is:
\begin{equation}\label{eq:cqr-Q}
  Q = Q_{1-\alpha}^{w}(E_{1:n}),
\end{equation}
where $Q^w$ denotes the (optionally Wasserstein-reweighted) quantile
with finite-sample correction from \eqref{eq:q-correction}.

The prediction interval in normalised space is:
\begin{equation}\label{eq:cqr-interval}
  \hat{C}_t = \bigl[\hat{Y}_t + (\hat{q}_{\mathrm{lo}} - Q)
    \cdot \hat{\sigma}_t,\;
  \hat{Y}_t + (\hat{q}_{\mathrm{hi}} + Q) \cdot \hat{\sigma}_t\bigr],
\end{equation}
yielding asymmetric margins:
\begin{align}\label{eq:cqr-margins}
  m^{-} &= Q - \hat{q}_{\mathrm{lo}} \ge 0, \\
  m^{+} &= \hat{q}_{\mathrm{hi}} + Q \ge 0.
\end{align}

\paragraph{Key advantages over separate quantiles.}
\begin{enumerate}
  ...
  \item \textbf{Interval tightening.}  When the base quantiles
    $[\hat{q}_{\mathrm{lo}}, \hat{q}_{\mathrm{hi}}]$ are already
    over-conservative (covering more than $1-\alpha$), the CQR correction
    $Q$ becomes \emph{negative}, actively \emph{shrinking} the interval.
    This is impossible with separate non-negative quantile thresholds.
  \item \textbf{Natural asymmetry.}  The base quantiles capture the
    skewness of the residual distribution (e.g., leverage effects in
    financial data), while $Q$ provides a symmetric conformal correction.
\end{enumerate}

\paragraph{Compatibility with Wasserstein reweighting.}
Both the base quantile estimates~\eqref{eq:base-quantiles} and the
conformal threshold~\eqref{eq:cqr-Q} use the same importance
weights~\eqref{eq:wass-weight}, maintaining consistency under
distribution shift.  The additive spectral margin is disabled when
CQR scoring is active, as the single-score mechanism already adapts
to shift via the reweighted quantile.
...
\begin{enumerate}
  ...
  \item \textbf{CQR-Inspired Single-Score Conformal Correction.}
    Adaptation of the CQR principle~\citep{romano2019conformalized} to
    the online setting \emph{without} a quantile regression model.
    Empirical base quantiles from the sliding calibration
    window~\eqref{eq:base-quantiles} serve as ``pseudo quantile
    regression'' estimates, and a single nonconformity
    score~\eqref{eq:cqr-score} yields exact finite-sample coverage via
    one threshold~$Q$~\eqref{eq:cqr-Q}.  Crucially, $Q$ can be
    \emph{negative}, enabling the conformal correction to \emph{shrink}
    over-conservative base intervals---a property unavailable when two
    separate quantile thresholds are used.
\end{enumerate}
